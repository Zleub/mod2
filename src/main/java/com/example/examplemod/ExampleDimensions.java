...
import com.example.examplemod.data.generators.biome.ExampleBiomeBuilders;
...
import static com.example.examplemod.data.generators.biome.ExampleBiomeBuilders.buildAetherBiomeSource;
...
//        BiomeSource source = ExampleBiomeBuilders.buildAetherBiomeSource(biomes);
//        HolderSet<Biome> test = HolderSet.direct(biomes);

//        HolderSet<Biome> test =
//        if (context.registryLookup(Registries.BIOME).isPresent()) {

//            HolderLookup.RegistryLookup<Biome> test = context.registryLookup(Registries.BIOME).get();
//            HolderSet.Direct<Biome> directBiomeSource = HolderSet.direct(test.listElements().map(e -> biomes.getOrThrow(e.key())).collect(Collectors.toList()));

//        Biomes.BADLANDS

//            Codec<HolderSet<Biome>> test = Biome.LIST_CODEC.;

//            test.listOf().
            BiomeSource source = buildAetherBiomeSource(biomes);
            NoiseBasedChunkGenerator aetherChunkGen = new NoiseBasedChunkGenerator(source, noiseSettings.getOrThrow(NoiseGeneratorSettings.OVERWORLD));
